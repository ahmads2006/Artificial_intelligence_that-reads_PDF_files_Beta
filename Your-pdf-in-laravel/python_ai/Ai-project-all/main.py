...
class AdvancedPDFQA:
    ...
    def load_pdf(self, path: str, doc_id: Optional[str] = None) -> bool:
        ...
        try:
            ...
            logger.info(f"PDF loaded successfully in {load_duration:.2f}s - {num_pages} pages, {len(full_text):,} chars")
            return True
    ...
    def load_multiple_pdfs(self, paths: List[str]) -> int:
        ...
        logger.info(f"Loaded {success_count}/{len(paths)} documents successfully")
...
Answer:"""
                ...
                # Multi-chunk search
                logger.info(f"Searching {len(chunks)} document sections...")
                best_answer = None
                best_confidence = 0.0
        ...
        try:
            ...
            if self.use_ai and self.ai_provider.available:
                ...
                if (confidence < 0.3 or "not found" in answer.lower() or "error" in answer.lower()
                    or "quota" in answer.lower() or "exceeded" in answer.lower()):
                    logger.info("AI failed or low confidence, trying traditional search")
...
def api_mode(pdf_path: str, question: str) -> None:
    """API mode for Laravel integration"""
    try:
        # Initialize QA system with API key for AI functionality
        api_key = os.getenv("GEMINI_API_KEY")
        qa = AdvancedPDFQA(api_key=api_key, use_ai=True)

        # Load PDF
        if not qa.load_pdf(pdf_path):
            result = json.dumps({
...
if __name__ == "__main__":
    if len(sys.argv) == 3:
        # API mode: python main.py <pdf_path> <question>
        pdf_path = sys.argv[1]
        question = sys.argv[2]
        api_mode(pdf_path, question)
    else:
        # Interactive mode
        interactive_mode()
